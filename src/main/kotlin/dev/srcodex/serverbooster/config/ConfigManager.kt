...
class ConfigManager(private val plugin: ServerBoosterPlugin) {
    ...
    lateinit var tpsCommandsConfig: TpsCommandsConfig
        private set
    lateinit var chunkBlockLimitsConfig: ChunkBlockLimitsConfig
        private set
    ...
    fun loadAll() {
        ...
        chunkBlockLimitsConfig = ChunkBlockLimitsConfig.load(java.io.File(plugin.dataFolder, "chunk_block_limits.yml"))
    ...
    private fun saveDefaultConfigs() {
        ...
        saveResourceIfNotExists("chunk_block_limits.yml")
    ...
    private fun loadChunkOptimizerConfig(): ChunkOptimizerConfig {
        ...
        return ChunkOptimizerConfig(
            ...
            unloadChunksInterval = config.getInt("unload-chunks.interval-ticks", 600),
            unloadChunksInactiveTimeout = config.getInt("unload-chunks.inactive-timeout-seconds", 300),
            unloadChunksMinDistance = config.getInt("unload-chunks.min-unload-distance", 10),
            unloadChunksSpawnRadius = config.getInt("unload-chunks.spawn-chunk-radius", 2),
            unloadChunksActivityDecay = config.getInt("unload-chunks.activity-decay-percent", 10),
            unloadChunksLog = config.getBoolean("unload-chunks.log.unload", true),
            unloadChunksDebug = config.getBoolean("unload-chunks.log.debug", false),
